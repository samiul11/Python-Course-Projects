...
RE_RANKED_FILE = "data/sdm_re_ranked2.csv"
...
for q_id in list(queries_dict.keys()):
    ...
    sdm_scores.sort_values(ascending=False, inplace=True)
    sdm_scores = sdm_scores.reset_index()
    sdm_scores.insert(0, 'QueryId', q_id)
    sdm_scores.drop(columns=[0], inplace=True)
    sdm_scores.columns = ['QueryId', 'EntityId']
    re_ranked_dfs.append(sdm_scores)
